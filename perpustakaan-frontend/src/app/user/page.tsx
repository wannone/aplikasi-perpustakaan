...
export default function User() {
  ...
  useEffect(() => {
    if (isEdit && edit) {
      const fetchUserData = async () => {
        try {
          if (token) {
            const data = await GetUserById(edit, token);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      if (isEdit && edit && token) {
        const request = await UpdateUser(values, edit, token);
        if (request) {
          toast({
            title: "Success",
                ...
                >
                            ...
                            <SelectContent>
                              {roles.map((item) => (
                                <SelectGroup key={item.role_id}>
                                  <SelectItem value={item.role_id.toString()}>
                                    {item.nama}
                                  </SelectItem>
                                </SelectGroup>
                              ))}
                  ...
                  <Button
                    ...
                    className={cn(
                      "w-full",
                      "text-white",
                      isEdit ? "bg-amber-400 hover:bg-amber-500" : " ",
                      "transition",
                      "duration-300"
                    )}
                    disabled={!isEdit}
